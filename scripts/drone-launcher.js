...
Events.on(ClientLoadEvent, cons(e => {
    ...
    launcherBlock.buildType = () => extend(PowerTurret.PowerTurretBuild, launcherBlock, {
        ...
        setTier(val) { 
            ...
            else if (val == 1) this.health = 1800; 
            else if (val == 2) this.health = 2400;
            this.maxHealth = this.health;
        },

        getMaxDrones() {
            return this.getTier() == 1 ? 6 : 4;
        },
        ...
        getMasterSwordUnitType() {
            return getUnitType(["cargo-master-sword-drone", "newex-cargo-master-sword-drone"]);
        },

        getSwordUnitType() {
            return getUnitType(["newex-cargo-sword-drone", "cargo-sword-drone", "cargo-sworddrone"]);
        },

        getMK2UnitType() {
            return getUnitType(["newex-cargo-drone-mk2", "cargo-drone-mk2", "newex-cargo-dronemk2", "cargo-dronemk2"]);
        },

        getNormalUnitType() {
            return getUnitType(["newex-combat-drone", "combat-drone", "newex-cargo-drone", "cargo-drone"]);
        },

        isMyDrone(u, types) {
            if (u.dead || u.team != this.team) return false;
            if (global.droneLauncherMap[u.id] !== this.id) return false;
            for (let i = 0; i < types.length; i++) {
                if (types[i] != null && u.type == types[i]) return true;
            }
            return false;
        },
        ...
        countActiveDrones() {
            let count = 0;
            let types = [
                this.getNormalUnitType(),
                this.getMK2UnitType(),
                this.getSwordUnitType(),
                this.getMasterSwordUnitType()
            ];

            Groups.unit.each(u => {
                if (this.isMyDrone(u, types)) count++;
            });
            return count;
        },
        ...
        countActiveMK2Drones() {
            let count = 0;
            let mk2Type = this.getMK2UnitType();

            Groups.unit.each(u => {
                if (this.isMyDrone(u, [mk2Type])) count++;
            });
            return count;
        },
        ...
        updateTile() {
            ...
            if (this.doorTimer > 0) {
                this.doorTimer = Math.max(0, this.doorTimer - Time.delta);
            }
        ...
        spawnCombatDrone() {
            ...
            if (droneType == null) {
                ...
                if (Mathf.chance(0.50) && swordType != null) {
                    droneType = swordType;
                } else {
                ...
                buttonTable.button(Icon.upOpen, Styles.cleari, 40, packRun(() => {
                    ...
                    b1.button("[green]KÍCH HOẠT MK2[]", packRun(() => {
                        ...
                        if (core != null && core.items.get(Items.copper) >= reqMK2.copper && core.items.get(Items.lead) >= reqMK2.lead && core.items.get(Items.silicon) >= reqMK2.silicon) {
                            ...
                            core.items.remove(Items.lead, reqMK2.lead); 
                    ...
                    b2.button("[orange]KÍCH HOẠT MK2B[]", packRun(() => {
                        ...
                        if (core != null && core.items.get(Items.copper) >= reqMK2B.copper && core.items.get(Items.lead) >= reqMK2B.lead && core.items.get(Items.titanium) >= reqMK2B.titanium) {
                            ...
                            core.items.remove(Items.lead, reqMK2B.lead); 
