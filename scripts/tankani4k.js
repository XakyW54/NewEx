...
// Yêu cầu tài nguyên nâng cấp cấu hình tháp pháo (Đã tăng 50%)
...
const crumbleExplosionEffect = new Effect(40, cons(e => {
    ...
    // LỚP 3: Hiệu ứng biến mất từ trong ra (Hollow Out / Expand Fade)
...
// ==================== ĐỊNH NGHĨA ĐẠN HỆ THỐNG (ĐÃ TĂNG 50% DMG) ====================

// Đạn gốc của Tankani-4k (MK1)
...
Events.on(ClientLoadEvent, cons(e => {
    // Lấy block tháp pháo Tankani-4k từ hệ thống
    ...
    if (turretBlock != null) {
                ...
                return new Bar(
                    new Prov({ 
                        get: function(){ 
                            ...
                            let baseText = "DMG: +" + Math.floor(e.getStackRatio() * 80) + "%";
                            if(e.damageStack >= maxStack) {
                    ...
                    new Prov({ 
                        get: function(){ 
                            return e.damageStack >= maxStack ? Color.cyan : Color.orange; 
                        } 
                    }),
                    new Floatp({ 
                        get: function(){ 
        ...
        turretBlock.buildType = () => extend(ItemTurret.ItemTurretBuild, turretBlock, {
            ...
            // Hàm hỗ trợ lấy tỷ lệ tầng phục vụ hiển thị thanh Bar
            getStackRatio() {
            ...
            setTier(val) { 
                this.tierState = val; // ĐÃ SỬA: Cập nhật lại giá trị tierState khi nhận config từ mạng hoặc UI
                // Chỉ số máu tháp pháo tăng 50%
                ...
                if (tier == 0) {
                    table.button(Icon.upOpen, Styles.cleari, 40, packRun(() => {
                        ...
                        let b1D = b1.add("Tích hợp công nghệ nổ mảnh diện rộng:\n" +
                                         ...
                                         " [white]• Mục tiêu chính nhận [red]1673 sát thương gốc[] trực diện.[]\n" +
                                         " [white]• Kẻ địch lân cận chịu [yellow]837 sát thương nổ lan[].[]");
                        ...
                        let b2D = b2.add("Chuyển đổi sang tổ hợp pháo phòng không hạng nặng:\n" +
                                         ...
                                         " [white]• Gây [orange]1338 sát thương diện rộng[].[]");
                ...
                table.button(Icon.info, Styles.cleari, 40, packRun(() => {
                    ...
                    if (currentTier == 0) {
                        ...
                        descStr = "[purple]⚡ CẤU HÌNH PHÒNG KHÔNG HẠNG NẶNG (MK2B) ⚡[]\n" +
                                  ...
                                  "[lightgray]Sát thương nổ phòng không:[] [orange]" + this.getModifiedDamage(1338) + " Sát thương[] (Gốc: 1338)\n" +
                                  statStackStr +
                                  "[scarlet]⚠ Giới hạn đặt: Tối đa 1 cấu trúc/đội[]";
                    }
            ...
            // Cấu hình thuộc tính đạn động và bổ sung cơ chế Xuyên giáp / Giảm giáp
            shoot(type) {
                let tier = this.getTier();
                let activeSpecial = (this.damageStack >= maxStack); // Kiểm tra xem đã đủ tăng 80% sát thương chưa

                if (tier == 1) {
                    tankaniMK2Bullet.damage = this.getModifiedDamage(1673);
                    tankaniMK2Bullet.splashDamage = this.getModifiedDamage(837);
                    
                    // Cơ chế xuyên giáp và thay đổi tỷ lệ giáp nhận vào mục tiêu thay vì gán status ăn mòn
            ...
            updateTile() {
                ...
                if (this.limitCheck >= 15) { 
                    ...
                    if (count > 1 && this !== firstBuild) {
                        Call.sendMessage("[red]Giới hạn: Chỉ được đặt tối đa 1 tháp pháo Tankani-4k! Cấu trúc thừa đã tự hủy![]"); 
                        this.kill(); 
                        return;
                    }
                }
